...
namespace AdventOfCode
{
    class Day6
    {
        ...
        {
            var groups = InputData.Split(Environment.NewLine + Environment.NewLine);
            var res = 0;
            foreach (var group in groups)
            {

                var answers = group.Select(x => x.ToString()).Where(x => x != "\r" && x != "\n").Distinct().ToArray();
                res += answers.Length;
            }
            return res;
        }
        ...
        public static int Part2()
        {
            ...
            foreach (var group in groups)
            {
                ...
                var allPossible = answers.SelectMany(x => x).Select(x => x.ToString()).ToHashSet();

                res += allPossible.Where(x => answers.All(y => y.Contains(x))).Count();
            }
            return res;
        }

        private static string InputData = @"jmqnkzlsfedaptx
usjfkadqwmeyilph
