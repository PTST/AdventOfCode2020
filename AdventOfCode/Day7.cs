...
namespace AdventOfCode
{
    class Day7
    {
        public static int Part1()
        {
            ...
            {
                bags.Add(new Bag(item));
            }
            foreach (var bag in bags)
            {
                bag.AddBags(bags);
            }
            return bags.Where(x => x.CanHoldShinyGold).Count();
        }
        ...
        public static int Part2()
        {
            ...
            return bags.First(x => x.IsShinyGold).RequiredBagsContained;
        }

        private static string InputData = @"plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
...
bright brown bags contain 2 dotted green bags, 4 clear tan bags, 3 wavy red bags.";
    ...
    class Bag
    {
        ...
        public Dictionary<Bag, int> CanContain { get; set; } = new Dictionary<Bag, int>();

        private MatchCollection CanContainRules { get; set; }
        public Bag(string input)
        {
            var match1 = new Regex(@"^(.*?) bags contain (.*?)$").Match(input);
            this.Name = match1.Groups[1].Value;
            this.CanContainRules = new Regex(@"(\d) (.*?) (bag|bags)(,|.)").Matches(match1.Groups[2].Value);
        }

        public int RequiredBagsContained
        {
            get
            {
                int count = 0;
                ...
                {
                    count += bag.Value;
                    count += bag.Key.RequiredBagsContained* bag.Value;
                }
                return count;
            }
        }
        ...
        public bool IsShinyGold
        {
            get
            {
                return this.Name == "shiny gold";
            }
        }

        public bool CanHoldShinyGold 
        { 
            get
            {
                foreach (var item in CanContain.Keys)
                {
                    if (item.IsShinyGold || item.CanHoldShinyGold)
                    {
                        return true;
                    }
                }
        ...
        public void AddBags(IEnumerable<Bag> otherBags)
        {
            foreach (Match item in CanContainRules)
            {
                var bag = otherBags.First(x => x.Name == item.Groups[2].Value);
                this.CanContain[bag] = int.Parse(item.Groups[1].Value);
            }
        }
    }
}
