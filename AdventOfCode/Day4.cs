...
namespace AdventOfCode
{
    class Passport
    {
        private Regex HeightSplitter = new Regex(@"(\d+)(in|cm)");
        private Regex HairColorValidator = new Regex(@"^#[0-9a-f]{6}$");
        ...
        public bool IsValid
        {
            get
            {
                return !string.IsNullOrEmpty(this.BirthYear)
                ...
                //&& !string.IsNullOrEmpty(this.CountryID);
            }
        }

        public bool IsExtraValid
        {
            get
            {
                return IsValid
        ...
        private bool ValidatePassportNo()
        {
            return this.PassportID.Length == 9 && this.PassportID.All(char.IsNumber);
        }

        private bool ValidateEyeColor()
        {
            var allowerdValues = new string[] { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };
            return allowerdValues.Contains(this.EyeColor);
        }

        private bool ValidateHairColor()
        {
            return this.HairColorValidator.IsMatch(this.HairColor);
        }

        private bool ValidateHeight()
        {
            var height = this.Height;
            if (this.HeightSplitter.IsMatch(height))
            {
                var splitHeight = this.HeightSplitter.Match(height);
                var heightNo = int.Parse(splitHeight.Groups[1].Value);
                if (splitHeight.Groups[2].Value == "cm")
                {
                    return heightNo >= 150 && heightNo <= 193;
                }
                return heightNo >= 59 && heightNo <= 76;
            }
            return false;
        }
        ...
        {
            var year = 0;
            if (int.TryParse(this.IssueYear, out year))
            {
        ...
        {
            var year = 0;
            if (int.TryParse(this.ExpirationYear, out year))
            {
        ...
        {
            this.BirthYear = input.GetValueOrDefault("byr");
            this.IssueYear = input.GetValueOrDefault("iyr");
            this.ExpirationYear = input.GetValueOrDefault("eyr");
            this.Height = input.GetValueOrDefault("hgt");
            this.HairColor = input.GetValueOrDefault("hcl");
            this.EyeColor = input.GetValueOrDefault("ecl");
            this.PassportID = input.GetValueOrDefault("pid");
            this.CountryID = input.GetValueOrDefault("cid");
        }
    }
    ...
    class Day4
    {
        ...
        {
            var passports = ParseInput(InputData);
            return passports.Where(x => x.IsValid).Count();
        }

        public static int Part2()
        {
            var passports = ParseInput(InputData);
            return passports.Where(x => x.IsExtraValid).Count();
        }


        private static List<Passport> ParseInput(string input)
        {
            var returnData = new List<Passport>();
            var data = input.Split(Environment.NewLine + Environment.NewLine);
            foreach (var passport in data)
            {
                var returnDict = new Dictionary<string, string>();
                foreach (var kv in passport.Split(new string[] { Environment.NewLine, " " }, StringSplitOptions.RemoveEmptyEntries))
                {
                    var x = kv.Split(":");
                    returnDict[x[0]] = x[1];
                }
                returnData.Add(new Passport(returnDict));
            }
            return returnData;
        }

        private static string InputData = @"eyr:2027
hcl:#602927
