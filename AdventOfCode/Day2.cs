...
namespace AdventOfCode
{
    class Day2
    {
        ...
        {
            var input = InputData.Split(Environment.NewLine).Select(x => SplitInput(x));
            return input.Where(x => PasswordChecker(x.password, x.character, x.min, x.max)).Count();
        }

        public static int Part2()
        {
            var input = InputData.Split(Environment.NewLine).Select(x => SplitInput(x)).ToArray();
            return input.Where(x => PasswordChecker2(x.password, x.character, x.min, x.max)).Count();
        }

        private static bool PasswordChecker(string password, string mustContainCharacter, int minCount, int maxCount)
        {
            var matches = new Regex(mustContainCharacter, RegexOptions.IgnoreCase).Matches(password);
            return matches.Count >= minCount && matches.Count <= maxCount;
        }

        private static bool PasswordChecker2(string password, string mustContainCharacter, int pos1, int pos2)
        {
            var check1 = password[pos1 - 1].ToString().ToLower() == mustContainCharacter.ToLower() ;
            var check2 = password[pos2 - 1].ToString().ToLower() == mustContainCharacter.ToLower();
            return check1 ^ check2;
        }

        private static (int min, int max, string character, string password) SplitInput(string input)
        {
            var split1 = input.Split(": ");
            var passWord = split1[1];
            var split2 = split1[0].Split(" ");
            var character = split2[1];
            var split3 = split2[0].Split("-");
            var min = int.Parse(split3[0]);
            var max = int.Parse(split3[1]);
            return (min, max, character, passWord);
        }

        private static string InputData = @"6-8 s: svsssszslpsp
3-4 n: gncn
